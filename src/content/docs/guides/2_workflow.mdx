...
The Author always catalyzes QC with the **inital qc commit**. This "starting line" for the QCer is posted to the Issue when it's initialized via the "Assign QC file(s)" button in the Assign app.

The QCer gets the "last word" with the **approved QC commit** posted via the "Approve" button in the Status app.

Commits between these two endpoints are *not* manditory. 
...
The diagram below outlines the ideal posted QC commit timeline pattern for ghqc Issues.
...
1. The QCer clones the repository if necessary.
...
2. The QCer confirms that their repository is configured for QC with the Status shiny app.
    ...
    She can confirm she's reviewing the correct QC file version in her local repo by verifying its "Up to date" Git Status in the Status app.
