...
commands to install the ecosystem and clone an example configuration repository.
...
2. Install ghqc and its suggested dependencies
    ...
    ```
3. Install auxillary dependencies and configuration
    ```r
    ghqc::ghqc_example_setup()
    ```
4. [Run the apps](#run-the-apps)
</Steps>
...
<Aside>
  {
    <>
      For more information on <code>rv</code>, check out the {' '}
      <a
        href="https://github.com/A2-ai/rv?tab=readme-ov-file#rv"
        target="_blank"
        rel="noopener noreferrer"
        style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', whiteSpace: 'nowrap' }}
      >
        GitHub repo <Icon name="external" />
      </a>.
    </>
  }
</Aside>
...
5. Install ghqc.app
    ...
    Due to organizational requirements for package management and installation, the package orchestrating the Shiny apps (`ghqc.app`) is not installed as part of `ghqc::setup_ghqc()`.\
    Install `ghqc.app` with the following command:
...
To run the apps, ensure your project is open and is an initialzed git repository with a remote upstream.
