...
<Tabs syncKey="new">
    <TabItem label = "First time setup">
        ...
        [3;4;35mhttps://github.com/a2-ai/ghqc.example_info_repo
        ```
    </TabItem>
    <TabItem label = "Repeat setup">
        ...
        [0mGHQC_INFO_REPO is set to [3;4;0mhttps://github.com/a2-ai/ghqc.example_info_repo
        [0min your ~/.Renviron
        [35mCustom Information Repository ([3;4;35mhttps://github.com/a2-ai/ghqc.example_info_repo)
        ```
    </TabItem>
</Tabs>
...
[35mWould you like to install `gert` to continue? (y/N)
```
</Aside>
...
(~/.local/share/ghqc/[3;4;35mexample_info_repo[0;35m)
```
...
```ansi
[32m✔[0m logo.png successfully found

[32m✔[0m 'note' successfully found

    [3;0m“Checklist items should be editted by the author on GitHub after
    ...
    items on a case-per-case basis.”
    — [0m
    
[32m✔[0m Checklists directory successfully found
...
#### Installing the ghqc.app dependencies
...
<Tabs>
    <TabItem label = "INSTALL">
        If you are on a linux operating system, ghqc will install [the dependencies](reference/ghqc.app-dependencies) from [PPM@2024-03-01](https://packagemanager.posit.co/cran/2024-03-01)
    ...
    <TabItem label = "LINK">
        If you have a package library previously installed, containing [all of the package versions required](reference/ghqc.app-dependencies), [symlinking](https://servicenow.iu.edu/kb?id=kb_article_view&sysparm_article=KB0023928) will the fastet option as it does not have to install anything.

        After entering `2`, you will be prompted to where to symlink [the dependency packages](reference/ghqc.app-dependencies) from on your system.

        ```ansi
        [35mPath to previously installed package library from which to link:
        ```

        You will then be prompted with where to symlink [the dependencies](reference/ghqc.app-dependencies) to. To set the destintation as the default directory, which is *highly* recommended, hit `ENTER`.
        If a different location is required, you are able to enter the file path.
